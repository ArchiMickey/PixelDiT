...
def PixelDiT_Tiny_4(**kwargs):
    return PixelDiT_Tiny(semantic_patch_size=4, compress_ratio=4, **kwargs)

def create_pixeldit_model(model_name, **kwargs):
    if model_name.lower() == 'pixeldit-tiny/4':
        return PixelDiT_Tiny_4(**kwargs)
    raise ValueError(f"Unsupported PixelDiT model: {model_name}")