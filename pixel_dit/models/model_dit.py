...
def DiT_Tiny_4(**kwargs):
    return DiT_Tiny(patch_size=4, **kwargs)

def create_dit_model(model_name, **kwargs):
    if model_name.lower() == 'dit-tiny/4':
        return DiT_Tiny_4(**kwargs)
    raise ValueError(f"Unsupported DiT model: {model_name}")